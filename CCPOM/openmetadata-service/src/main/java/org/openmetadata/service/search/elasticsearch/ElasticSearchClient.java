...
public class ElasticSearchClient implements SearchClient {
  ...
  public Response search(SearchRequest request) throws IOException {
    ...
    switch (request.getIndex()) {
      ...
      case "glossary_term_search_index":
        ...
        break;
      case "knowledge_center_document_search_index":
        searchSourceBuilder =
            buildKnowledgeCenterDocumentSearchBuilder(request.getQuery(), request.getFrom(), request.getSize());
        break;
      case "client_center_client_search_index":
        searchSourceBuilder =
            buildClientCenterClientSearchBuilder(request.getQuery(), request.getFrom(), request.getSize());
        break;
      case "tag_search_index":
        searchSourceBuilder = buildTagSearchBuilder(request.getQuery(), request.getFrom(), request.getSize());
        break;
      case "dla_search_index":
        searchSourceBuilder = buildDlaSearchBuilder(request.getQuery(), request.getFrom(), request.getSize());
        break;
      case "spd_search_index":
        searchSourceBuilder = buildSpdSearchBuilder(request.getQuery(), request.getFrom(), request.getSize());
        break;
      case "autosystem_search_index":
        searchSourceBuilder = buildAutoSystemSearchBuilder(request.getQuery(), request.getFrom(), request.getSize());
        break;
      case "useresk_search_index":
        searchSourceBuilder = buildUserEskSearchBuilder(request.getQuery(), request.getFrom(), request.getSize());
        break;
      ...
      case "data_product_search_index":
        ...
        break;
      case "perimeter_search_index":
        searchSourceBuilder = buildPerimeterSearch(request.getQuery(), request.getFrom(), request.getSize());
        break;
      default:
    ...
    if (request.getIndex().equalsIgnoreCase("all")) {
      ...
      searchSourceBuilder.query(boolQueryBuilder);
        ...
        || request.getIndex().equalsIgnoreCase("perimeter_search_index")
    ...
    if (request.getIndex().equalsIgnoreCase("knowledge_center_document_search_index")) {
      searchSourceBuilder.query(
          QueryBuilders.boolQuery()
              .must(searchSourceBuilder.query())
              .must(QueryBuilders.matchQuery("status", "Approved")));
    ...
    if (request.getIndex().equalsIgnoreCase("client_center_client_search_index")) {
      searchSourceBuilder.query(
          QueryBuilders.boolQuery()
              .must(searchSourceBuilder.query())
              .must(QueryBuilders.matchQuery("status", "Approved")));
  ...
  private static SearchSourceBuilder buildKnowledgeCenterDocumentSearchBuilder(String query, int from, int size) {
    QueryStringQueryBuilder queryBuilder =
        QueryBuilders.queryStringQuery(query)
            .fields(KnowledgeCenterDocumentIndex.getFields())
            .defaultOperator(Operator.AND)
            .fuzziness(Fuzziness.AUTO);

    HighlightBuilder.Field highlightKnowledgeCenterName = new HighlightBuilder.Field(FIELD_NAME);
    highlightKnowledgeCenterName.highlighterType(UNIFIED);
    HighlightBuilder.Field highlightKnowledgeCenterDisplayName = new HighlightBuilder.Field(FIELD_DISPLAY_NAME);
    highlightKnowledgeCenterDisplayName.highlighterType(UNIFIED);
    HighlightBuilder.Field highlightDescription = new HighlightBuilder.Field(FIELD_DESCRIPTION);
    highlightDescription.highlighterType(UNIFIED);
    HighlightBuilder.Field highlightSynonym = new HighlightBuilder.Field("synonyms");
    highlightDescription.highlighterType(UNIFIED);
    HighlightBuilder hb = new HighlightBuilder();
    hb.field(highlightDescription);
    hb.field(highlightKnowledgeCenterName);
    hb.field(highlightKnowledgeCenterDisplayName);
    hb.field(highlightSynonym);
    ...
    SearchSourceBuilder searchSourceBuilder =
        new SearchSourceBuilder().query(queryBuilder).highlighter(hb).from(from).size(size);
    searchSourceBuilder.aggregation(
        AggregationBuilders.terms("knowledgeCenter.name.keyword").field("knowledgeCenter.name.keyword"));
    return addAggregation(searchSourceBuilder);
  }

  private static SearchSourceBuilder buildClientCenterClientSearchBuilder(String query, int from, int size) {
    QueryStringQueryBuilder queryBuilder =
        QueryBuilders.queryStringQuery(query)
            .fields(ClientCenterClientIndex.getFields())
            .defaultOperator(Operator.AND)
            .fuzziness(Fuzziness.AUTO);

    HighlightBuilder.Field highlightClientCenterName = new HighlightBuilder.Field(FIELD_NAME);
    highlightClientCenterName.highlighterType(UNIFIED);
    HighlightBuilder.Field highlightClientCenterDisplayName = new HighlightBuilder.Field(FIELD_DISPLAY_NAME);
    highlightClientCenterDisplayName.highlighterType(UNIFIED);
    HighlightBuilder.Field highlightDescription = new HighlightBuilder.Field(FIELD_DESCRIPTION);
    highlightDescription.highlighterType(UNIFIED);
    HighlightBuilder.Field highlightSynonym = new HighlightBuilder.Field("synonyms");
    highlightDescription.highlighterType(UNIFIED);
    HighlightBuilder hb = new HighlightBuilder();
    hb.field(highlightDescription);
    hb.field(highlightClientCenterName);
    hb.field(highlightClientCenterDisplayName);
    hb.field(highlightSynonym);
    ...
    SearchSourceBuilder searchSourceBuilder =
        new SearchSourceBuilder().query(queryBuilder).highlighter(hb).from(from).size(size);
    searchSourceBuilder.aggregation(
        AggregationBuilders.terms("clientCenter.name.keyword").field("clientCenter.name.keyword"));
    return addAggregation(searchSourceBuilder);
  }
  ...
  private static SearchSourceBuilder buildDlaSearchBuilder(String query, int from, int size) {
    QueryStringQueryBuilder queryBuilder =
        QueryBuilders.queryStringQuery(query)
            .fields(DlaIndex.getFields())
            .defaultOperator(Operator.AND)
            .fuzziness(Fuzziness.AUTO);

    HighlightBuilder.Field highlightDlaName = new HighlightBuilder.Field(FIELD_NAME);
    highlightDlaName.highlighterType(UNIFIED);
    HighlightBuilder.Field highlightDlaDisplayName = new HighlightBuilder.Field(FIELD_DISPLAY_NAME);
    highlightDlaDisplayName.highlighterType(UNIFIED);
    HighlightBuilder.Field highlightDescription = new HighlightBuilder.Field(FIELD_DESCRIPTION);
    highlightDescription.highlighterType(UNIFIED);
    HighlightBuilder hb = new HighlightBuilder();
    hb.field(highlightDlaDisplayName);
    hb.field(highlightDescription);
    hb.field(highlightDlaName);
    hb.preTags(PRE_TAG);
    hb.postTags(POST_TAG);
    SearchSourceBuilder searchSourceBuilder =
        new SearchSourceBuilder().query(queryBuilder).highlighter(hb).from(from).size(size);
    searchSourceBuilder.aggregation(
        AggregationBuilders.terms("dlaClassification.name.keyword").field("dlaClassification.name.keyword"));
    return addAggregation(searchSourceBuilder);
  }

  private static SearchSourceBuilder buildSpdSearchBuilder(String query, int from, int size) {
    QueryStringQueryBuilder queryBuilder =
        QueryBuilders.queryStringQuery(query)
            .fields(SpdIndex.getFields())
            .defaultOperator(Operator.AND)
            .fuzziness(Fuzziness.AUTO);

    HighlightBuilder.Field highlightSpdName = new HighlightBuilder.Field(FIELD_NAME);
    highlightSpdName.highlighterType(UNIFIED);
    HighlightBuilder.Field highlightSpdDisplayName = new HighlightBuilder.Field(FIELD_DISPLAY_NAME);
    highlightSpdDisplayName.highlighterType(UNIFIED);
    HighlightBuilder.Field highlightDescription = new HighlightBuilder.Field(FIELD_DESCRIPTION);
    highlightDescription.highlighterType(UNIFIED);
    HighlightBuilder hb = new HighlightBuilder();
    hb.field(highlightSpdDisplayName);
    hb.field(highlightDescription);
    hb.field(highlightSpdName);
    hb.preTags(PRE_TAG);
    hb.postTags(POST_TAG);
    SearchSourceBuilder searchSourceBuilder =
        new SearchSourceBuilder().query(queryBuilder).highlighter(hb).from(from).size(size);
    searchSourceBuilder.aggregation(
        AggregationBuilders.terms("spdClassification.name.keyword").field("spdClassification.name.keyword"));
    return addAggregation(searchSourceBuilder);
  }

  private static SearchSourceBuilder buildAutoSystemSearchBuilder(String query, int from, int size) {
    QueryStringQueryBuilder queryBuilder =
        QueryBuilders.queryStringQuery(query)
            .fields(AutoSystemIndex.getFields())
            .defaultOperator(Operator.AND)
            .fuzziness(Fuzziness.AUTO);

    HighlightBuilder.Field highlightAutoSystemName = new HighlightBuilder.Field(FIELD_NAME);
    highlightAutoSystemName.highlighterType(UNIFIED);
    HighlightBuilder.Field highlightAutoSystemDisplayName = new HighlightBuilder.Field(FIELD_DISPLAY_NAME);
    highlightAutoSystemDisplayName.highlighterType(UNIFIED);
    HighlightBuilder.Field highlightDescription = new HighlightBuilder.Field(FIELD_DESCRIPTION);
    highlightDescription.highlighterType(UNIFIED);
    HighlightBuilder hb = new HighlightBuilder();
    hb.field(highlightAutoSystemDisplayName);
    hb.field(highlightDescription);
    hb.field(highlightAutoSystemName);
    hb.preTags(PRE_TAG);
    hb.postTags(POST_TAG);
    SearchSourceBuilder searchSourceBuilder =
        new SearchSourceBuilder().query(queryBuilder).highlighter(hb).from(from).size(size);
    searchSourceBuilder.aggregation(
        AggregationBuilders.terms("autoSystemClassification.name.keyword")
            .field("autoSystemClassification.name.keyword"));
    return addAggregation(searchSourceBuilder);
  }

  private static SearchSourceBuilder buildUserEskSearchBuilder(String query, int from, int size) {
    QueryStringQueryBuilder queryBuilder =
        QueryBuilders.queryStringQuery(query)
            .fields(UserEskIndex.getFields())
            .defaultOperator(Operator.AND)
            .fuzziness(Fuzziness.AUTO);

    HighlightBuilder.Field highlightUserEskName = new HighlightBuilder.Field(FIELD_NAME);
    highlightUserEskName.highlighterType(UNIFIED);
    HighlightBuilder.Field highlightUserEskDisplayName = new HighlightBuilder.Field(FIELD_DISPLAY_NAME);
    highlightUserEskDisplayName.highlighterType(UNIFIED);
    HighlightBuilder.Field highlightDescription = new HighlightBuilder.Field(FIELD_DESCRIPTION);
    highlightDescription.highlighterType(UNIFIED);
    HighlightBuilder hb = new HighlightBuilder();
    hb.field(highlightUserEskDisplayName);
    hb.field(highlightDescription);
    hb.field(highlightUserEskName);
    hb.preTags(PRE_TAG);
    hb.postTags(POST_TAG);
    SearchSourceBuilder searchSourceBuilder =
        new SearchSourceBuilder().query(queryBuilder).highlighter(hb).from(from).size(size);
    searchSourceBuilder.aggregation(
        AggregationBuilders.terms("userEskClassification.name.keyword").field("userEskClassification.name.keyword"));
    return addAggregation(searchSourceBuilder);
  }
  ...
  private static SearchSourceBuilder buildPerimeterSearch(String query, int from, int size) {
    QueryStringQueryBuilder queryBuilder =
        QueryBuilders.queryStringQuery(query)
            .fields(PerimeterIndex.getFields())
            .defaultOperator(Operator.AND)
            .fuzziness(Fuzziness.AUTO);
