...
import static org.openmetadata.service.util.EntityUtil.domainDataSourcesListMatch;
import static org.openmetadata.service.util.EntityUtil.domainQuotasListMatch;
import static org.openmetadata.service.util.EntityUtil.domainTeamMatch;
...
public class DomainRepository extends EntityRepository<Domain> {
  private static final String UPDATE_FIELDS =
      "parent,children,experts,fullName,domainOwner,domainTechLeader,dataSourcesList,domainTeam,domainQuotasList";
  ...
  public class DomainUpdater extends EntityUpdater {
    ...
    public void entitySpecificUpdate() {
      ...
      recordChange("domainOwner", original.getDomainOwner(), updated.getDomainOwner());
      recordChange("domainTechLeader", original.getDomainTechLeader(), updated.getDomainTechLeader());
      updateDataSourcesList(original, updated);
      updateDomainTeam(original, updated);
      updateDomainQuotasList(original, updated);
    }

    private void updateDataSourcesList(Domain original, Domain updated) {
      List<DataSourceEl> origIndicators = listOrEmpty(original.getDataSourcesList());
      List<DataSourceEl> updatedIndicators = listOrEmpty(updated.getDataSourcesList());

      List<DataSourceEl> added = new ArrayList<>();
      List<DataSourceEl> deleted = new ArrayList<>();
      recordListChange(
          "dataSourcesList", origIndicators, updatedIndicators, added, deleted, domainDataSourcesListMatch);
    }

    private void updateDomainTeam(Domain original, Domain updated) {
      List<DomainTeamMember> origIndicators = listOrEmpty(original.getDomainTeam());
      List<DomainTeamMember> updatedIndicators = listOrEmpty(updated.getDomainTeam());

      List<DomainTeamMember> added = new ArrayList<>();
      List<DomainTeamMember> deleted = new ArrayList<>();
      recordListChange("domainTeam", origIndicators, updatedIndicators, added, deleted, domainTeamMatch);
    }

    private void updateDomainQuotasList(Domain original, Domain updated) {
      List<DomainQuota> origIndicators = listOrEmpty(original.getDomainQuotasList());
      List<DomainQuota> updatedIndicators = listOrEmpty(updated.getDomainQuotasList());

      List<DomainQuota> added = new ArrayList<>();
      List<DomainQuota> deleted = new ArrayList<>();
      recordListChange("domainQuotasList", origIndicators, updatedIndicators, added, deleted, domainQuotasListMatch);
    }
  }
}
