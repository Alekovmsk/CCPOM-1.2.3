...
export const getEntityLinkFromType = (
  ...
  switch (entityType) {
    ...
    case EntityType.KNOWLEDGE_CENTER_DOCUMENT:
      return getKnowledgeCenterDocumentDetailsPath(fullyQualifiedName);
    case EntityType.CLIENT_CENTER:
    case EntityType.CLIENT_CENTER_CLIENT:
      return getClientCenterClientDetailsPath(fullyQualifiedName);
    case EntityType.TAG:
      return getTagsDetailsPath(getEncodedFqn(fullyQualifiedName));
    case EntityType.DLA:
      return getDlasDetailsPath(getEncodedFqn(fullyQualifiedName));
    case EntityType.SPD:
      return getSpdsDetailsPath(getEncodedFqn(fullyQualifiedName));
    case EntityType.AUTOSYSTEM:
      return getAutoSystemsDetailsPath(getEncodedFqn(fullyQualifiedName));
    case EntityType.USERESK:
      return getUserEsksDetailsPath(getEncodedFqn(fullyQualifiedName));
    case EntityType.TOPIC:
    ...
    case EntityType.PERIMETER:
      return getPerimetersDetailsPath(getEncodedFqn(fullyQualifiedName));
    case EntityType.DASHBOARD_DATA_MODEL:
...
export const getEntityBreadcrumbs = (
  ...
  switch (entityType) {
    ...
    case EntityType.KNOWLEDGE_CENTER_DOCUMENT:
      // eslint-disable-next-line no-case-declarations
      const knowledgeCenter = (entity as KnowledgeCenterDocument).knowledgeCenter;
      if (!knowledgeCenter) {
        return [];
      }
      // eslint-disable-next-line no-case-declarations
      const fqnKnowledgeCenterList = Fqn.split((entity as KnowledgeCenterDocument).fullyQualifiedName);
      // eslint-disable-next-line no-case-declarations
      const treeKnowledgeCenter = fqnKnowledgeCenterList.slice(1, fqnKnowledgeCenterList.length);

      return [
        {
          name: knowledgeCenter.fullyQualifiedName,
          url: getKnowledgeCenterPath(knowledgeCenter.fullyQualifiedName),
        },
        ...treeKnowledgeCenter.map((fqn, index, source) => ({
          name: fqn,
          url: getKnowledgeCenterPath(
            `${knowledgeCenter.fullyQualifiedName}.${source
              .slice(0, index + 1)
              .join('.')}`
          ),
        })),
    ...
    case EntityType.CLIENT_CENTER_CLIENT:
      // eslint-disable-next-line no-case-declarations
      const clientCenter = (entity as ClientCenterClient).clientCenter;
      if (!clientCenter) {
        return [];
      }
      // eslint-disable-next-line no-case-declarations
      const fqnClientCenterList = Fqn.split((entity as ClientCenterClient).fullyQualifiedName);
      // eslint-disable-next-line no-case-declarations
      const treeClientCenter = fqnClientCenterList.slice(1, fqnClientCenterList.length);

      return [
        {
          name: clientCenter.fullyQualifiedName,
          url: getClientCenterPath(clientCenter.fullyQualifiedName),
        },
        ...treeClientCenter.map((fqn, index, source) => ({
          name: fqn,
          url: getClientCenterPath(
            `${clientCenter.fullyQualifiedName}.${source
              .slice(0, index + 1)
              .join('.')}`
          ),
        })),
    ...
    case EntityType.DLA:
      // eslint-disable-next-line no-case-declarations
      const fqnDlaList = Fqn.split((entity as Dla).fullyQualifiedName);

      return [
        ...fqnDlaList.map((fqn) => ({
          name: fqn,
          url: getDlasDetailsPath(entity?.fullyQualifiedName ?? ''),
        })),
      ];
    case EntityType.SPD:
      // eslint-disable-next-line no-case-declarations
      const fqnSpdList = Fqn.split((entity as Spd).fullyQualifiedName);

      return [
        ...fqnSpdList.map((fqn) => ({
          name: fqn,
          url: getSpdsDetailsPath(entity?.fullyQualifiedName ?? ''),
        })),
      ];
    case EntityType.AUTOSYSTEM:
      // eslint-disable-next-line no-case-declarations
      const fqnAutoSystemList = Fqn.split((entity as AutoSystem).fullyQualifiedName);

      return [
        ...fqnAutoSystemList.map((fqn) => ({
          name: fqn,
          url: getAutoSystemsDetailsPath(entity?.fullyQualifiedName ?? ''),
        })),
      ];
    case EntityType.USERESK:
      // eslint-disable-next-line no-case-declarations
      const fqnUserEskList = Fqn.split((entity as UserEsk).fullyQualifiedName);

      return [
        ...fqnUserEskList.map((fqn) => ({
          name: fqn,
          url: getUserEsksDetailsPath(entity?.fullyQualifiedName ?? ''),
        })),
      ];
    ...
    case EntityType.PERIMETER: {
      const data = entity as Perimeter;
      if (!data.domain) {
        return [];
      }
