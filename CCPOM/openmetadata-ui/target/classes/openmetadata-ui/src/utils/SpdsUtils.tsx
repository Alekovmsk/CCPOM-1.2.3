...
export const getEntityReferenceFromSpd = (
  spd: Spd
): EntityReference => {
  return {
    deleted: spd.deleted,
    href: spd.href,
    fullyQualifiedName: spd.fullyQualifiedName ?? '',
    id: spd.id,
    type: 'spd',
...
export const getTaglist = async (
  classifications: Array<SpdClassification> = [],
  paging?: Paging
) => {
  try {
    const tags: Spd[] = [];
...
export const fetchSpdsElasticSearch = async (
  searchText: string,
  page: number,
  ...
  const res = await searchQuery({
    ...
    pageSize: PAGE_SIZE,
    queryFilter: {},
    searchIndex: SearchIndex.SPD,
  });

  return {
    data: res.hits.hits.reduce((result: ResultType[], { _source }) => {
      const classificationName =
        _source.spdClassification?.fullyQualifiedName ?? '';
...
export const createTierTag = (tag: Spd) => {
  return {
...
export const updateTierTag = (oldTags: Spd[] | TagLabel[], newTier?: Spd) => {
  return newTier
