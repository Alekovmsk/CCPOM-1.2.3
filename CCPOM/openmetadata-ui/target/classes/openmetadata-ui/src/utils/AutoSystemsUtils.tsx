...
export const getClassifications = async (
  ...
  try {
    const listOfClassifications: Array<AutoSystemClassification> = [];
    const classifications = await getAllClassifications(fields, 1000);
    const classificationList = classifications.data.map(
      (category: AutoSystemClassification) => {
        return {
          name: category.name,
          description: category.description,
        } as AutoSystemClassification;
      }
    );
    if (classificationList.length && callGetClassificationByName) {
      const promiseArr = classificationList.map((category: AutoSystemClassification) =>
...
export const getEntityReferenceFromAutoSystem = (
  autoSystem: AutoSystem
): EntityReference => {
  return {
    deleted: autoSystem.deleted,
    href: autoSystem.href,
    fullyQualifiedName: autoSystem.fullyQualifiedName ?? '',
    id: autoSystem.id,
    type: 'autoSystem',
    description: autoSystem.description,
    displayName: autoSystem.displayName,
...
export const getTaglist = async (
  classifications: Array<AutoSystemClassification> = [],
  paging?: Paging
) => {
  try {
    const tags: AutoSystem[] = [];
...
export const fetchAutoSystemsElasticSearch = async (
  searchText: string,
  page: number,
  ...
  return {
    data: res.hits.hits.reduce((result: ResultType[], { _source }) => {
      const classificationName =
        _source.autoSystemClassification?.fullyQualifiedName ?? '';
...
export const createTierTag = (tag: AutoSystem) => {
  return {
...
export const updateTierTag = (oldTags: AutoSystem[] | TagLabel[], newTier?: AutoSystem) => {
  return newTier
