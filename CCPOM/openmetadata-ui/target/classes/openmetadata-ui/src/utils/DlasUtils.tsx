...
export const getEntityReferenceFromDla = (
  dla: Dla
): EntityReference => {
  return {
    deleted: dla.deleted,
    href: dla.href,
    fullyQualifiedName: dla.fullyQualifiedName ?? '',
    id: dla.id,
    type: 'dla',
    description: dla.description,
    displayName: dla.displayName,
    name: dla.name,
  };
};
...
export const getTaglist = async (
  classifications: Array<DlaClassification> = [],
  paging?: Paging
) => {
  try {
    const tags: Dla[] = [];
...
export const fetchDlasElasticSearch = async (
  searchText: string,
  page: number,
  ...
  const res = await searchQuery({
    ...
    pageSize: PAGE_SIZE,
    queryFilter: {},
    searchIndex: SearchIndex.DLA,
  });

  return {
    data: res.hits.hits.reduce((result: ResultType[], { _source }) => {
      const classificationName =
        _source.dlaClassification?.fullyQualifiedName ?? '';
...
export const createTierTag = (tag: Dla) => {
  return {
...
export const updateTierTag = (oldTags: Dla[] | TagLabel[], newTier?: Dla) => {
  return newTier
