...
import KnowledgeCenterDocumentReferencesModal from '../KnowledgeCenterDocumentReferencesModal.component';

interface KnowledgeCenterDocumentReferencesProps {
  isVersionView?: boolean;
  knowledgeCenterDocument: KnowledgeCenterDocument;
  permissions: OperationPermission;
  onKnowledgeCenterDocumentUpdate: (knowledgeCenterDocument: KnowledgeCenterDocument) => void;
}

const KnowledgeCenterDocumentReferences = ({
  knowledgeCenterDocument,
  permissions,
  onKnowledgeCenterDocumentUpdate,
  isVersionView,
}: KnowledgeCenterDocumentReferencesProps) => {
  const [references, setReferences] = useState<DocumentReference[]>([]);
  const [isViewMode, setIsViewMode] = useState<boolean>(true);

  const handleReferencesSave = async (
    newReferences: DocumentReference[],
    updateState?: boolean
  ) => {
    try {
      ...
      if (!isEqual(updatedRef, knowledgeCenterDocument.references)) {
        ...
        onKnowledgeCenterDocumentUpdate(updatedKnowledgeCenterDocument);
        if (updateState) {
  ...
  const onReferenceModalSave = (values: DocumentReference[]) => {
    handleReferencesSave(values);
  };

  useEffect(() => {
    setReferences(knowledgeCenterDocument.references ? knowledgeCenterDocument.references : []);
  }, [knowledgeCenterDocument.references]);

  const getReferenceElement = useCallback(
    (ref: DocumentReference, versionStatus?: VersionStatus) => {
      ...
      return (
        <Tag
          className={classNames(
            'm-r-xs m-t-xs d-flex items-center document-reference-tag bg-white',
  ...
  const getVersionReferenceElements = useCallback(() => {
    ...
    const addedReferences: DocumentReference[] = JSON.parse(
      getChangedEntityNewValue(referencesDiff) ?? '[]'
    );
    const deletedReferences: DocumentReference[] = JSON.parse(
      getChangedEntityOldValue(referencesDiff) ?? '[]'
    );

    const unchangedReferences = knowledgeCenterDocument.references
      ? knowledgeCenterDocument.references.filter(
          (reference) =>
            !addedReferences.find(
              (addedReference: DocumentReference) =>
  ...
  return (
    <div data-testid="references-container">
            ...
            <div className="d-flex flex-wrap">
              ...
              {permissions.EditAll && references.length === 0 && (
                <TagButton
                  ...
                  dataTestId="document-references-add-button"
      ...
      <KnowledgeCenterDocumentReferencesModal
        isVisible={!isViewMode}
        references={references || []}
        onClose={() => {
          setIsViewMode(true);
        }}
        onSave={(values: DocumentReference[]) => {
...
export default KnowledgeCenterDocumentReferences;
