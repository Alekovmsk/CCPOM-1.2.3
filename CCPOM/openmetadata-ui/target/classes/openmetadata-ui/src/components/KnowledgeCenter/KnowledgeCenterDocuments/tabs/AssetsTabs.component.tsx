...
import {
  ...
  removeAssetsFromKnowledgeCenterDocument,
  ...
  (
    {
      ...
      type = AssetsOfEntity.KNOWLEDGE_CENTER,
      noDataPlaceholder,
      entityFqn,
      assetCount,
    }: AssetsTabsProps,
    ...
    const isRemovable = useMemo(
      () =>
        [
          ...
          AssetsOfEntity.KNOWLEDGE_CENTER,
        ].includes(type),
      [type]
    );
    ...
    const [activeEntity, setActiveEntity] = useState<
      Domain | DataProduct | KnowledgeCenterDocument
    >();
    ...
    const entityTypeString =
      type === AssetsOfEntity.KNOWLEDGE_CENTER
        ? t('label.knowledge-center-term-lowercase')
    ...
    const queryParam = useMemo(() => {
      ...
      switch (type) {
        case AssetsOfEntity.DOMAIN:
          return `(domain.fullyQualifiedName:"${encodedFqn}") AND !(entityType:"dataProduct") AND !(entityType:"perimeter")`;
    ...
    const fetchAssets = useCallback(
      async ({
        ...
        try {
          ...
          console.log(quickFilterQuery);
          console.log(entityFqn)
          console.log(typeof entityFqn)
          const quickFilter2={"query":{"bool":{"must":[{"bool":{"should":[{"term":{"tags.tagFQN":entityFqn}}]}}]}}}
    ...
    const fetchCurrentEntity = useCallback(async () => {
      ...
      switch (type) {
        ...
        case AssetsOfEntity.KNOWLEDGE_CENTER:
          data = await getKnowledgeCenterDocumentByFQN(getDecodedFqn(fqn));
      ...
      async (assetsData: SourceType[]) => {
        ...
        try {
          ...
          switch (type) {
            ...
            case AssetsOfEntity.KNOWLEDGE_CENTER:
              await removeAssetsFromKnowledgeCenterDocument(
                activeEntity as KnowledgeCenterDocument,
                entities
              );
