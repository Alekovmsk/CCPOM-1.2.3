    ...
    () =>
      tag.source === TagSource.Glossary
        ...
        : (tag.source === TagSource.Classification 
          ? history.push(`${ROUTES.TAGS}/${getEncodedFqn(Fqn.split(tag.tagFQN)[0])}`) 
          : (tag.source === TagSource.Dlaclassification 
            ? history.push(`${ROUTES.DLAS}/${getEncodedFqn(Fqn.split(tag.tagFQN)[0])}`) 
            : (tag.source === TagSource.Autosystemclassification 
              ? history.push(`${ROUTES.AUTOSYSTEMS}/${getEncodedFqn(Fqn.split(tag.tagFQN)[0])}`) 
              : (tag.source === TagSource.KnowledgeCenter 
                ? history.push(`${ROUTES.KNOWLEDGE_CENTER}/${getEncodedFqn(Fqn.split(tag.tagFQN)[0])}`) 
                : (tag.source === TagSource.ClientCenter 
                  ? history.push(`${ROUTES.CLIENT_CENTER}/${getEncodedFqn(Fqn.split(tag.tagFQN)[0])}`) 
                  : history.push(`${ROUTES.SPDS}/${getEncodedFqn(Fqn.split(tag.tagFQN)[0])}`)))))),
