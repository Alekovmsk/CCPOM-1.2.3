...
import { getClientCenterClientByFQN, getClientCentersByName } from '../../../rest/clientCenterAPI';
import TagsClientCenterClientV1 from '../TagsV1/TagsV1ClientCenterClient.component';
import { ClientCenter } from '../../../generated/entity/data/clientCenter';
import { showErrorToast } from '../../../utils/ToastUtils';
import { AxiosError } from 'axios';

const TagsViewerClientCenter: FunctionComponent<TagsViewerProps> = ({
  tags,
  sizeCap = LIST_SIZE,
  ...
  const [tagsFull,setTagsFull] = useState<any>();

  const getTagsElement =useCallback(
    (tag: EntityTags) => (
      <>
      <TagsClientCenterClientV1
  ...
  const fetchClientCenterClientDetails = async (tag:EntityTags) => {
    try {
      // setIsTagsLoading(true);
      const response = await getClientCenterClientByFQN(
        tag.tagFQN
      );
      setTagsFull(response);
    } catch (error) {
  ...
  useEffect(() => {
    if (tags[0])
    {fetchClientCenterClientDetails(tags[0])}
    else
    {setTagsFull(undefined)}
  },[tags])
  ...
  return (
    <>
      {(tagsFull && (tags[0]?.added || tags[0]?.removed)) &&
      <>
      ...
      {(tagsFull && !(tags[0]?.added || tags[0]?.removed)) && <><TagsClientCenterClientV1
        className={classNames(
          { 'diff-added tw-mx-1': tagsFull?.added },
          { 'diff-removed': tagsFull?.removed }
        )}
        isVersionPage={tagsFull?.added || tagsFull?.removed}
        showOnlyName={tagsFull.source === TagSource.Glossary}
        key={tagsFull.fullyQualifiedName}
        startWith={TAG_START_WITH.SOURCE_ICON}
        tag={tagsFull}
        isTagLabel={false}
      /></>}
...
export default TagsViewerClientCenter;
