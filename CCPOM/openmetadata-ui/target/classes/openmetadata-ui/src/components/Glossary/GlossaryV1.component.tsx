...
const GlossaryV1 = ({
  ...
  const fetchGlossaryTerm = async (
    ...
    try {
      const termsResponse = await getGlossaryTerms({
        ...
        limit: PAGE_SIZE_TERMS,
      });
      const AllTermsData = glossaryTerms.concat(termsResponse.data);
      setGlossaryTerms(AllTermsData);
      setPaging(termsResponse.paging);
    } catch (error) {
  ...
  const fetchMoreTerms = useCallback(
    ...
    [setIsLoading, fetchGlossaryTerm]
  );

  // useEffect(() => {
  //   fetchMoreTerms(isInView, CursorType.AFTER);
  // }, [isInView]);
      ...
      {!isLoading &&
        ...
        (isGlossaryActive ? (
          <GlossaryDetails
            ...
            moreload={() => fetchMoreTerms(true, CursorType.AFTER)}
          />
        ) : (
          <GlossaryTermsV1
