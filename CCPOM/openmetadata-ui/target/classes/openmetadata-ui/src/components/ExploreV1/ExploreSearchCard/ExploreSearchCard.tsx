...
>(
  (
    ...
    const otherDetails = useMemo(() => {
      ...
      if (source.entityType === EntityType.CLIENT_CENTER_CLIENT) {
        const _otherDetailsClientCenter: ExtraInfo[] = []
        if ('fullName' in source) {
          _otherDetailsClientCenter.push({
            value: source.fullName,
          });
        };
        if ('dateOfBirth' in source) {
          _otherDetailsClientCenter.push({
        ...
        return _otherDetailsClientCenter;
      };
      console.log('sourceVal');
      console.log(source);

      if (source.entityType === EntityType.KNOWLEDGE_CENTER_DOCUMENT) {
        const _otherDetailsKnowledgeCenter: ExtraInfo[] = []
        if ('agreementType' in source) {
          _otherDetailsKnowledgeCenter.push({
            value: source.agreementType,
          });
        };
        if ('agreementOperator' in source) {
          _otherDetailsKnowledgeCenter.push({
            value: source.agreementOperator,
          });
        };
        if ('agreementContact' in source) {
          _otherDetailsKnowledgeCenter.push({
            value: source.agreementContact,
          });
        };
        if ('agreementValidity' in source) {
          _otherDetailsKnowledgeCenter.push({
            value: source.agreementValidity,
          });
        };
        return _otherDetailsKnowledgeCenter;
      }
    ...
    const header = useMemo(() => (
      <Row gutter={[8, 8]}>
        ...
        <Col
          data-testid={`${source.service?.name ? `${source.service.name}-` : 'explore-card-'}${source.name}`}
          span={24}>
          {isTourOpen ? (
            ...
            <div className="w-full d-flex items-start">
              ...
              <Link
                ...
                to={source.fullyQualifiedName && source.entityType
                  ? getEntityLinkFromType(
