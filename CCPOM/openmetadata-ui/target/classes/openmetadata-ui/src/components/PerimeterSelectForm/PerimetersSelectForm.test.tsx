...
import PerimetersSelectForm from './PerimetersSelectForm';

const mockOnSubmit = jest.fn();
const mockOnCancel = jest.fn();

describe('Perimeters Form Page', () => {
  it('renders without errors', () => {
    const { getByTestId } = render(
      <PerimetersSelectForm
        defaultValue={[]}
        fetchApi={() => Promise.resolve({ data: [], paging: { total: 0 } })}
        placeholder="Select Perimeters"
    ...
    expect(getByTestId('perimeter-selector')).toBeInTheDocument();
  });
  ...
  it('calls onSubmit when the Save button is clicked', () => {
    ...
    const selectRef = getByTestId('perimeter-selector').querySelector(
      '.ant-select-selector'
    );
    if (selectRef) {
