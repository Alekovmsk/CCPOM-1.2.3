 ...
 */
import { Badge, Button, DatePicker, Space, Tooltip, Typography } from 'antd';
...
import ClassificationDetails from '../../components/DlaClassificationDetails/DlaClassificationDetails';
...
import {
  CreateDla,
  ProviderType,
} from '../../generated/api/dlaClassification/createDla';
...
import { getDlaPath } from '../../utils/RouterUtils';
import { getDecodedFqn, getErrorText } from '../../utils/StringsUtils';
import { showErrorToast } from '../../utils/ToastUtils';
import TagsForm from './DlasForm';
...
const TagsPage = () => {
  ...
  const [currentClassificationName, setCurrentClassificationName] =
    useState<string>('');
  const [tags, setTags] = useState<Dla[]>();
  ...
  const fetchClassificationChildren = async (
    ...
    try {
      const tagsResponse = await getTags({
        arrQueryFields: ['usageCount','owner','autoSourceSystem', 'autoConsumerSystem'],
  ...
  const fetchClassifications = async (setCurrent?: boolean) => {
    ...
    try {
      ...
      if (setCurrent && response.data.length) {
        ...
        history.push(getDlaPath(response.data[0].fullyQualifiedName));
      }
    } catch (error) {
      const errMsg = getErrorText(
        error as AxiosError,
  ...
  const handleCreateClassification = async (data: CreateDlaClassification) => {
    setIsButtonLoading(true);
    try {
      const res = await createClassification(data);
      await fetchClassifications();
      history.push(getDlaPath(res.fullyQualifiedName));
    } catch (error) {
      if (
  ...
  const handleUpdateClassification = async (
    updatedClassification: DlaClassification
  ) => {
    if (!isUndefined(currentClassification)) {
      ...
      try {
        ...
        if (
          ...
          history.push(getDlaPath(response.fullyQualifiedName));
        }
      } catch (error) {
        if (
          (error as AxiosError).response?.status === HTTP_STATUS_CODE.CONFLICT
  ...
  const handleCreatePrimaryTag = async (data: CreateDla) => {
    try {
      await createTag({
        ...data,
        dlaClassification: currentClassification?.fullyQualifiedName,
      });
  ...
  const handleUpdatePrimaryTag = async (updatedData: Dla) => {
  ...
  const handleActionDeleteTag = (record: Dla) => {
    if (currentClassification) {
      setDeleteTags({
        data: {
  ...
  const handleEditTagClick = (selectedTag: Dla) => {
    setIsAddingTag(true);
  ...
  const onClickClassifications = (category: DlaClassification) => {
    setCurrentClassification(category);
    setCurrentClassificationName(category.fullyQualifiedName ?? '');
    history.push(getDlaPath(category.fullyQualifiedName));
  };
  ...
  const leftPanelLayout = useMemo(
    () => (
      <LeftPanelCard id="tags" className=''>
        <TagsLeftPanelSkeleton loading={isLoading}>
          <div className="p-y-xs" data-testid="data-summary-container">
            ...
            {classifications.map((category: DlaClassification) => (
              <div
  ...
  const tagsFormHeader = useMemo(
    () =>
      editTag
        ...
        : t('label.add-entity', {entity: t('label.skd')}),
