...
import ClassificationDetails from '../../components/SpdClassificationDetails/SpdClassificationDetails';
...
import {
  CreateSpd,
  ProviderType,
} from '../../generated/api/spdClassification/createSpd';
...
import { getSpdPath } from '../../utils/RouterUtils';
import { getDecodedFqn, getErrorText } from '../../utils/StringsUtils';
import { showErrorToast } from '../../utils/ToastUtils';
import TagsForm from './SpdsForm';
...
const TagsPage = () => {
  ...
  const [currentClassificationName, setCurrentClassificationName] =
    useState<string>('');
  const [tags, setTags] = useState<Spd[]>();
  ...
  const fetchClassificationChildren = async (
    ...
    try {
      const tagsResponse = await getTags({
        arrQueryFields: ['usageCount','owner'],
  ...
  const fetchClassifications = async (setCurrent?: boolean) => {
    ...
    try {
      ...
      if (setCurrent && response.data.length) {
        ...
        history.push(getSpdPath(response.data[0].fullyQualifiedName));
      }
    } catch (error) {
      const errMsg = getErrorText(
        error as AxiosError,
  ...
  const handleCreateClassification = async (data: CreateSpdClassification) => {
    setIsButtonLoading(true);
    try {
      const res = await createClassification(data);
      await fetchClassifications();
      history.push(getSpdPath(res.fullyQualifiedName));
    } catch (error) {
      if (
  ...
  const handleUpdateClassification = async (
    updatedClassification: SpdClassification
  ) => {
    if (!isUndefined(currentClassification)) {
      ...
      try {
        ...
        if (
          ...
          history.push(getSpdPath(response.fullyQualifiedName));
        }
      } catch (error) {
        if (
          (error as AxiosError).response?.status === HTTP_STATUS_CODE.CONFLICT
  ...
  const handleCreatePrimaryTag = async (data: CreateSpd) => {
    try {
      await createTag({
        ...data,
        spdClassification: currentClassification?.fullyQualifiedName,
      });
  ...
  const handleUpdatePrimaryTag = async (updatedData: Spd) => {
  ...
  const handleActionDeleteTag = (record: Spd) => {
    if (currentClassification) {
      setDeleteTags({
        data: {
  ...
  const handleEditTagClick = (selectedTag: Spd) => {
    setIsAddingTag(true);
  ...
  const onClickClassifications = (category: SpdClassification) => {
    setCurrentClassification(category);
    setCurrentClassificationName(category.fullyQualifiedName ?? '');
    history.push(getSpdPath(category.fullyQualifiedName));
  };
      ...
      <LeftPanelCard id="tags" className=''>
        <TagsLeftPanelSkeleton loading={isLoading}>
          <div className="p-y-xs" data-testid="data-summary-container">
            ...
            {classifications.map((category: SpdClassification) => (
              <div
  ...
  const tagsFormHeader = useMemo(
    () =>
      editTag
        ...
        : t('label.add-entity', {entity: t('label.spd')}),
