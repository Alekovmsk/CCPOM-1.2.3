...
interface KnowledgeCenterDocumentSynonymsProps {
...
const KnowledgeCenterDocumentSynonyms = ({
  permissions,
  knowledgeCenterDocument,
  onKnowledgeCenterDocumentUpdate,
  isVersionView,
}: KnowledgeCenterDocumentSynonymsProps) => {
  ...
  const getSynonyms = () => (
    <div className="d-flex flex-wrap">
      {synonyms.map((synonym) => (
        <TagButton
          className="knowledge-center-synonym-tag"
          key={synonym}
          label={synonym}
        />
      ))}
  ...
  const getSynonymsContainer = useCallback(() => {
    ...
    const unchangedSynonyms = knowledgeCenterDocument.synonyms
      ? knowledgeCenterDocument.synonyms.filter(
          (synonym) =>
            !isEmpty(synonym) &&
            !addedSynonyms.find(
  ...
  }, [knowledgeCenterDocument, isVersionView, getSynonyms]);

  const handleCancel = () => {
    setSynonyms(knowledgeCenterDocument.synonyms || []);
    setIsViewMode(true);
  };

  const handleSynonymsSave = (newSynonyms: string[]) => {
    if (!isEqual(newSynonyms, knowledgeCenterDocument.synonyms)) {
  ...
  useEffect(() => {
    if (knowledgeCenterDocument.synonyms?.length) {
      // removing empty string
      setSynonyms(knowledgeCenterDocument.synonyms.filter((synonym) => !isEmpty(synonym)));
    }
  }, [knowledgeCenterDocument]);
    ...
    <div className="flex flex-col m-r-xs" data-testid="synonyms-container">
      ...
      {isViewMode ? (
        ...
        <>
          ...
          <Select
            className="knowledge-center-select w-full"
            id="synonyms-select"
            mode="tags"
            open={false}
...
export default KnowledgeCenterDocumentSynonyms;
