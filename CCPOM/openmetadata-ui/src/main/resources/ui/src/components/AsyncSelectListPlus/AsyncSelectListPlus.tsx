...
import {
  ...
  SelectOption,
} from './AsyncSelectListPlus.interface';

const AsyncSelectListPlus: FC<AsyncSelectListProps> = ({
  mode,
  onChange,
  fetchOptions,
  id,
  ...
  const handleChange: SelectProps['onChange'] = (values: string[], options) => {
    const selectedValues = (values.length != 0) ? [(values.map((value) => {
...
export default AsyncSelectListPlus;
