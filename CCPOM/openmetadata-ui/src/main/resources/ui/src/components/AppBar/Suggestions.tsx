...
const Suggestions = ({
  ...
  const [glossarySuggestions, setGlossarySuggestions] = useState<
    GlossarySource[]
  >([]);
  const [knowledgeCenterSuggestions, setKnowledgeCenterSuggestions] = useState<
    KnowledgeCenterSource[]
  >([]);
  const [clientCenterSuggestions, setClientCenterSuggestions] = useState<
    ClientCenterSource[]
  >([]);
  ...
  const [dlaSuggestions, setDlaSuggestions] = useState<DlaSource[]>([]);
  const [spdSuggestions, setSpdSuggestions] = useState<SpdSource[]>([]);
  const [autoSystemSuggestions, setAutoSystemSuggestions] = useState<AutoSystemSource[]>([]);
  const [userEskSuggestions, setUserEskSuggestions] = useState<UserEskSource[]>([]);
  ...
  const [dataProductSuggestions, setDataProductSuggestions] = useState<
    DataProductSource[]
  >([]);
  const [perimeterSuggestions, setPerimeterSuggestions] = useState<
    PerimeterSource[]
  >([]);
  ...
  const updateSuggestions = (options: Array<Option>) => {
    ...
    setKnowledgeCenterSuggestions(filterOptionsByIndex(options, SearchIndex.KNOWLEDGE_CENTER));
    setClientCenterSuggestions(filterOptionsByIndex(options, SearchIndex.CLIENT_CENTER));
    setTagSuggestions(filterOptionsByIndex(options, SearchIndex.TAG));
    setDlaSuggestions(filterOptionsByIndex(options, SearchIndex.DLA));
    setSpdSuggestions(filterOptionsByIndex(options, SearchIndex.SPD));
    setAutoSystemSuggestions(filterOptionsByIndex(options, SearchIndex.AUTOSYSTEM));
    setUserEskSuggestions(filterOptionsByIndex(options, SearchIndex.USERESK));
    setDataProductSuggestions(
      filterOptionsByIndex(options, SearchIndex.DATA_PRODUCT)
    );
    setPerimeterSuggestions(filterOptionsByIndex(options, SearchIndex.PERIMETER));
  };
    ...
    return (
      <div data-testid="global-search-suggestion-box" role="none">
        {[
          ...
          {
            suggestions: knowledgeCenterSuggestions,
            searchIndex: SearchIndex.KNOWLEDGE_CENTER,
          },
          {
            suggestions: clientCenterSuggestions,
            searchIndex: SearchIndex.CLIENT_CENTER,
          },
          { suggestions: tagSuggestions, searchIndex: SearchIndex.TAG },
          { suggestions: dlaSuggestions, searchIndex: SearchIndex.DLA },
          { suggestions: spdSuggestions, searchIndex: SearchIndex.SPD },
          { suggestions: autoSystemSuggestions, searchIndex: SearchIndex.AUTOSYSTEM },
          { suggestions: userEskSuggestions, searchIndex: SearchIndex.USERESK },
          {
            suggestions: dataProductSuggestions,
            searchIndex: SearchIndex.DATA_PRODUCT,
          },
          { suggestions: perimeterSuggestions, searchIndex: SearchIndex.PERIMETER },
