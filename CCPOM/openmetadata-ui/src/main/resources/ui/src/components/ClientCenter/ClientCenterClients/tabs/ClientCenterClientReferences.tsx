...
import ClientCenterClientReferencesModal from '../ClientCenterClientReferencesModal.component';

interface ClientCenterClientReferencesProps {
  isVersionView?: boolean;
  clientCenterClient: ClientCenterClient;
  permissions: OperationPermission;
  onClientCenterClientUpdate: (clientCenterClient: ClientCenterClient) => void;
}

const ClientCenterClientReferences = ({
  clientCenterClient,
  permissions,
  onClientCenterClientUpdate,
  isVersionView,
}: ClientCenterClientReferencesProps) => {
  const [references, setReferences] = useState<ClientReference[]>([]);
  const [isViewMode, setIsViewMode] = useState<boolean>(true);

  const handleReferencesSave = async (
    newReferences: ClientReference[],
    updateState?: boolean
  ) => {
    try {
      ...
      if (!isEqual(updatedRef, clientCenterClient.references)) {
        ...
        onClientCenterClientUpdate(updatedClientCenterClient);
        if (updateState) {
  ...
  const onReferenceModalSave = (values: ClientReference[]) => {
    handleReferencesSave(values);
  };

  useEffect(() => {
    setReferences(clientCenterClient.references ? clientCenterClient.references : []);
  }, [clientCenterClient.references]);

  const getReferenceElement = useCallback(
    (ref: ClientReference, versionStatus?: VersionStatus) => {
      ...
      return (
        <Tag
          className={classNames(
            'm-r-xs m-t-xs d-flex items-center client-reference-tag bg-white',
  ...
  const getVersionReferenceElements = useCallback(() => {
    ...
    const addedReferences: ClientReference[] = JSON.parse(
      getChangedEntityNewValue(referencesDiff) ?? '[]'
    );
    const deletedReferences: ClientReference[] = JSON.parse(
      getChangedEntityOldValue(referencesDiff) ?? '[]'
    );

    const unchangedReferences = clientCenterClient.references
      ? clientCenterClient.references.filter(
          (reference) =>
            !addedReferences.find(
              (addedReference: ClientReference) =>
  ...
  return (
    <div data-testid="references-container">
            ...
            <div className="d-flex flex-wrap">
              ...
              {permissions.EditAll && references.length === 0 && (
                <TagButton
                  ...
                  dataTestId="client-references-add-button"
      ...
      <ClientCenterClientReferencesModal
        isVisible={!isViewMode}
        references={references || []}
        onClose={() => {
          setIsViewMode(true);
        }}
        onSave={(values: ClientReference[]) => {
...
export default ClientCenterClientReferences;
