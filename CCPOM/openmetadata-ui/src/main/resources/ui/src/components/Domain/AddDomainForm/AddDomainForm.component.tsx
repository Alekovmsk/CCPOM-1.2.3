...
const AddDomainForm = ({
  ...
  let isPerimeter
  if (type !== DomainFormType.PERIMETER)
  {isPerimeter = false}
  else
  {isPerimeter = true}
  ...
  const formFields: FieldProp[] = [
    {
      name: 'name',
      id: 'root/name',
      label: isDomain ? t('label.mnemocode') + ' (' + t('label.name') + ')' : t('label.name'),
      required: true,
      placeholder: isDomain ? t('label.mnemocode') + ' (' + t('label.name') + ')' : t('label.name'),
      ...
      rules: [
        ...
        },
        {
          ...
          message: `${t('message.entity-maximum-size', {
            entity: isDomain ? `${t('label.mnemocode')}` : `${t('label.name')}`,
            max: '128',
          })}`,
        },
      ],
    ...
    },
    {
      name: 'description',
      required: true,
      label: isPerimeter ? t('label.attribute') : t('label.description'),
      id: 'root/description',
  ...
  if (type === DomainFormType.DOMAIN) {
    ...
    const domainFullNameField: FieldProp = {
      name: 'fullName',
      id: 'root/fullName',
      label: t('label.full-domain-name'),
      required: false,
      placeholder: t('label.full-domain-name'),
      type: FieldTypes.TEXT,
      props: {
        'data-testid': 'full-domain-name',
      },
    };

    formFields.push(domainTypeField);
    formFields.push(domainFullNameField);
  }
  ...
  const handleFormSubmit: FormProps['onFinish'] = (formData) => {
    ...
    const updatedData = omit(formData, 'color', 'iconURL', 'domainOwner', 'domainTechLeader');
    const style = {
    ...
    const data = {
      ...updatedData,
      style,
      domainOwner: formData.domainOwner ? formData.domainOwner[0].value : undefined,
      domainTechLeader: formData.domainTechLeader ? formData.domainTechLeader[0].value : undefined,
      experts: expertsList.map((item) => item.name ?? ''),
    } as CreateDomain | CreateDataProduct | CreatePerimeter;
            ...
            <>
              ...
              >
                <AsyncSelectListPlus
                  ...
                  mode={'multiple'}
                  placeholder={t('label.domain-owner')}
                />
              </Form.Item>
              <Form.Item
                label={t('label.domain-tech-leader')}
                className={'form-item-horizontal'}
                name={'domainTechLeader'}
              >
                <AsyncSelectListPlus
                  fetchOptions={fetchUserEsks}
                  mode={'multiple'}
                  placeholder={t('label.domain-tech-leader')}
                />
              </Form.Item>
            </>)}
          {/* {type === DomainFormType.DOMAIN && (
